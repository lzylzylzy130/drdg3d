...
FC := mpif90 -O3 -cpp -DpOrder=$(pOrder) -module $(OBJDIR) $(DFLAGS)
endif
ifeq "$(compiler)" "gnu"
...
######################################################
### for SUGON
#NETCDF_DIR := /public/software/mathlib/netcdf/intel/4.7.4
NETCDF_DIR := /public/software/mathlib/netcdf/intel/4.4.1
LAPACK_DIR := /public/software/mathlib/lapack/intel/3.10.0
LDFLAGS := -L $(LAPACK_DIR)/lib -llapack -lblas -L $(NETCDF_DIR)/lib -lnetcdff -lgfortran
#LDFLAGS := -llapack -L $(NETCDF_DIR)/lib -lnetcdff
...
######################################################
### for MacBook Pro
NETCDF_DIR := /opt/homebrew/Cellar/netcdf-fortran/4.6.0_1
#LDFLAGS := -lblas -llapack -L $(NETCDF_DIR)/lib -lnetcdff
#LDFLAGS := -L/usr/local/opt/openblas/lib -L/usr/local/opt/lapack/lib -L$(NETCDF_DIR)/lib -lnetcdff
LDFLAGS := -L/opt/homebrew/Cellar/openblas/0.3.23/lib -L/opt/homebrew/Cellar/lapack/3.11/lib -L$(NETCDF_DIR)/lib -L/opt/homebrew/Cellar/netcdf-fortran/4.6.0_1 -lnetcdff -lblas
#LDFLAGS := -lblas -llapack -L /usr/local/lib /usr/local/lib/libnetcdf.a /usr/local/lib/libnetcdff.a
#LDFLAGS := -L . -llapack -lrefblas
INC := -I $(NETCDF_DIR)/include
endif
